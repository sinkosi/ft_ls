...
{
	ft_print_d_int_fd(my_printf, c, fd);
}

static void	ft_handler_d_short(t_printf *my_printf, short n, int fd)
{
	ft_print_d_int_fd(my_printf, n, fd);
}

void		ft_handler_d_int_fd(t_printf *my_printf,
		va_list printf_list, int fd)
{
	...
	if (my_printf->h == 1)
		...
		ft_print_d_int_fd(my_printf, va_arg(printf_list, long), fd);
	else if (my_printf->l == 2)
		ft_print_d_int_fd(my_printf, va_arg(printf_list, long long), fd);
	else if (my_printf->j == 1)
		ft_print_d_int_fd(my_printf, va_arg(printf_list, intmax_t), fd);
	else if (my_printf->z == 1)
		ft_print_d_int_fd(my_printf, va_arg(printf_list, size_t), fd);
	else
		ft_print_d_int_fd(my_printf, va_arg(printf_list, int), fd);
}
