...
{
	ft_print_x_hexadecimal_fd(my_printf, n, fd);
}

static void	ft_handler_x_char(t_printf *my_printf, unsigned char c, int fd)
{
	ft_print_x_hexadecimal_fd(my_printf, c, fd);
}
		...
		va_list printf_list, int fd)
{
	...
	if (my_printf->h == 1)
		...
		ft_print_x_hexadecimal_fd(my_printf, va_arg(printf_list, long), fd);
	else if (my_printf->l == 2)
		ft_print_x_hexadecimal_fd(my_printf,
				va_arg(printf_list, unsigned long long), fd);
	else if (my_printf->j == 1)
		ft_print_x_hexadecimal_fd(my_printf,
				va_arg(printf_list, uintmax_t), fd);
	else if (my_printf->z == 1)
		ft_print_x_hexadecimal_fd(my_printf, va_arg(printf_list, size_t), fd);
	else
		ft_print_x_hexadecimal_fd(my_printf,
