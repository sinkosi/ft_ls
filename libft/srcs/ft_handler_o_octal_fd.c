...
{
	ft_print_o_octal_fd(my_printf, n, fd);
}

static void	ft_handler_o_char(t_printf *my_printf, unsigned char c, int fd)
{
	ft_print_o_octal_fd(my_printf, c, fd);
}

void		ft_handler_o_octal_fd(t_printf *my_printf,
		va_list printf_list, int fd)
{
	...
	if (my_printf->h == 1)
		...
		ft_print_o_octal_fd(my_printf, va_arg(printf_list, unsigned long), fd);
	else if (my_printf->l == 2)
		ft_print_o_octal_fd(my_printf,
				va_arg(printf_list, unsigned long long), fd);
	else if (my_printf->j == 1)
		ft_print_o_octal_fd(my_printf, va_arg(printf_list, uintmax_t), fd);
	else if (my_printf->z == 1)
		ft_print_o_octal_fd(my_printf, va_arg(printf_list, size_t), fd);
	else
		ft_print_o_octal_fd(my_printf, va_arg(printf_list, unsigned int), fd);
}
