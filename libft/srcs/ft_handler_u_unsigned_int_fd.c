...
{
	ft_print_u_unsigned_int_fd(my_printf, n, fd);
}

static void	ft_handler_u_char(t_printf *my_printf, unsigned char c, int fd)
{
	ft_print_u_unsigned_int_fd(my_printf, c, fd);
}
		...
		va_list printf_list, int fd)
{
	...
	if (my_printf->h == 1)
		...
		ft_print_u_unsigned_int_fd(my_printf,
		...
		ft_print_u_unsigned_int_fd(my_printf, va_arg(printf_list, size_t), fd);
	else
		ft_print_u_unsigned_int_fd(my_printf,
